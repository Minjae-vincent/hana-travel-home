<template>
  <v-container>
    <v-card class="pa-4">
      <v-switch
        v-model="enableAutoBuy"
        label="자동 구매 활성화"
        inset
        :class="enableAutoBuy ? 'info--text' : 'red--text'"
        :color="enableAutoBuy ? 'success' : 'red'"
      ></v-switch>
      <v-card-text class="auto-buy-class" :class="{ disabled: !enableAutoBuy }">
          ...
          <div class="d-flex align-center justify-space-between">
            ...
            <div class="d-flex align-center">
              <span class="highlight-text mr-2">
                {{ selectedRate ? selectedRate : rates[0].value }} 원 이하일 때
              </span>
          ...
          <div class="d-flex align-center justify-space-between">
            <div class="d-flex align-center">
              <span class="highlight-text"
                >{{ props.info.code }}
                {{
                  selectedRechargeAmount ? selectedRechargeAmount : rechargeOptions[0].jpyAmount
                }}</span
              >
              <span class="sub-text2 ml-1">충전</span>
            </div>
            <v-icon>mdi-chevron-down</v-icon>
          </div>
        </div>
        ...
        <div class="exchange-rate-box" @click="openPeriodSelection">
          ...
          <div class="d-flex align-center justify-space-between">
            <span class="highlight-text">
              {{ selectedPeriod ? selectedPeriod : '한 번만 충전' }}
            </span>
            <v-icon>mdi-chevron-down</v-icon>
          </div>
        </div>
        ...
        <div class="exchange-rate-box">
          <div class="label-text grey--text mb-2">충전방식</div>
          <div class="d-flex align-center justify-space-between">
            <span class="highlight-text"> 연결계좌에서 가져오기 </span>
          </div>
        </div>
        ...
        <v-bottom-sheet v-model="periodSheet">
          <v-sheet>
            <v-toolbar flat>
              <v-toolbar-title class="headline">충전 기간 선택</v-toolbar-title>
              <v-spacer></v-spacer>
            ...
            <v-list>
              <v-list-item-group v-model="selectedPeriod">
                ...
                >
                  <v-list-item-content>
                    <div v-if="option === '도달할 때마다 충전'">
                      <v-list-item-title class="period-option">
                        {{ option }}
                        <span class="sub-text">하루에 한 번만 충전</span>
                      </v-list-item-title>
                    </div>
                    <div v-else>
                      <v-list-item-title class="period-option">
        ...
        <v-bottom-sheet v-model="rechargeSheet">
          <v-sheet>
            <v-toolbar flat>
              <v-toolbar-title class="headline">얼마를 충전할까요?</v-toolbar-title>
              <v-spacer></v-spacer>
                ...
                >
                  <v-list-item-content>
                    <v-list-item-title class="recharge-option">
                      {{ props.info.code }} {{ formatNumber(option.jpyAmount) }}
                      <span class="sub-text">({{ formatNumber(option.krwAmount) }}원)</span>
                    </v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
              </v-list-item-group>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-text-field
                ...
                v-model="customRate"
                type="number"
                placeholder="자동 구매하고자 하는 환율을 입력해주세요."
              ></v-text-field>
            </v-card-text>
      ...
      <v-card-actions>
        <v-btn color="primary" block @click="setAutoBuy()">저장하기</v-btn>
      </v-card-actions>
    </v-card>
  </v-container>
...
const rates = ref([
  { value: '852.26', description: '최근 3개월 최저가' },
  { value: '905.79', description: '최근 1개월 최저가' },
  { value: '910.42', description: '최근 1주일 최저가' }
])

// const rates = ref([])

const rechargeOptions = ref([
  { jpyAmount: '5000', krwAmount: '42613' },
  { jpyAmount: '1000', krwAmount: '85226' },
  { jpyAmount: '20000', krwAmount: '170452' },
  { jpyAmount: '50000', krwAmount: '426130' },
  { jpyAmount: '30000', krwAmount: '255678' },
  { jpyAmount: '70000', krwAmount: '596582' },
  { jpyAmount: '100000', krwAmount: '852260' }
])

const getCurrencyInfo = async () => {
  try {
    await axios
      .get(`http://34.47.69.197:8081/api/user/manage/possession/auto-buy/${props.info.code}`)
      .then((response) => {
        ...
        if (props.info.code === 'JPY') {
          for (let i = 0; i < rechargeOptions.value.length; i++) {
            rechargeOptions.value[i].krwAmount = (
              (rechargeOptions.value[i].jpyAmount * currentRate.value) /
              100
            ).toFixed(0)
          }
        } else {
...
const setAutoBuy = async () => {
  ...
  try {
    await axios
      .post(`http://34.47.69.197:8081/api/user/manage/possession/auto-buy/${props.info.code}`, {
        memberId: userId,
        enableAuto: enableAutoBuy.value,
        ...
        rechargeAmount: selectedRechargeAmount.value
          ...
          : rechargeOptions.value[0].jpyAmount,
        period: realPeriod
      })
      .then((response) => {
        showNotification('자동 구매 설정이 저장되었습니다.', () => {
...
const getAutoBuyInfo = async () => {
  try {
    await axios
      .get(
        `http://34.47.69.197:8081/api/user/manage/possession/auto-buy/${props.info.code}/${userId}`
      )
      .then((response) => {
        enableAutoBuy.value = response.data.data.enableAuto
        selectedRate.value = response.data.data.exchangeRate
        selectedRechargeAmount.value = response.data.data.rechargeAmount
...
const periodOptions = ['한 번만 충전', '도달할 때마다 충전']
...
const selectRechargeAmount = (option) => {
  selectedRechargeAmount.value = option.jpyAmount
